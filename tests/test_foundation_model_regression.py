...
REFERENCE = {
    ...
    "GRACE-1L-SMAX-large": {
        ...
        "forces": np.array(
            [
                [-4.23692278e-01, -4.14788646e-16, 4.59755931e-16],
    ...
    "GRACE-1L-SMAX-OMAT-large": {
        ...
        "forces": np.array(
            [
                [-4.21008065e-01, -1.02714603e-16, -4.46555770e-17],
    ...
    "GRACE-2L-SMAX-large": {
        ...
        "forces": np.array(
            [
                [-4.12379417e-01, 5.63514079e-17, 1.68864488e-17],
    ...
    "GRACE-2L-SMAX-medium": {
        ...
        "forces": np.array(
            [
                [-4.19510346e-01, -2.49366500e-17, -8.51640806e-17],
    ...
    "GRACE-2L-SMAX-OMAT-large": {
        ...
        "forces": np.array(
            [
                [-3.98903191e-01, 4.62954328e-17, -7.95262294e-17],
    ...
    "GRACE-2L-SMAX-OMAT-medium": {
        ...
        "forces": np.array(
            [
                [-4.02579279e-01, 2.45029691e-17, -2.18846209e-16],
...
def test_grace_fm_prediction(model_name):
    ...
    assert np.allclose(
        e, ref["energy"], atol=1e-6
    ), f"Energy mismatch for {model_name}: {e} vs {ref['energy']}"
    assert np.allclose(f, ref["forces"], atol=1e-6), f"Forces mismatch for {model_name}"
    assert np.allclose(s, ref["stress"], atol=1e-6), f"Stress mismatch for {model_name}"
...
def test_checkpoint_matches_grace_fm(model_name):
    ...
    param_dtype = (
        get_dtype_by_name(meta["param_dtype"]) if "param_dtype" in meta else tf.float64
    )
    print("param_dtype", param_dtype)
    tp = TensorPotential(ins, param_dtype=param_dtype)
    ...
    assert np.allclose(f0, f1, atol=1e-5), f"Forces mismatch for {model_name}"
    assert np.allclose(s0, s1, atol=1e-5), f"Stress mismatch for {model_name}"
