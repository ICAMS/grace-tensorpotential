...
def simple_instructions():
    with InstructionManager() as instructor:
        ...
        SphericalHarmonic(vhat=rhat, lmax=2, name="Y")
        ScalarChemicalEmbedding(element_map={"H": 0}, embedding_size=8, name="Z")
...
def test_build_split_tpmodel():
    ...
    assert m.param_dtype == tf.float64
...
def test_infer_communicated_keys_omat_large_base():
    ...
    yaml_path = os.path.join(
        os.path.dirname(__file__), "model_grace_2L_omat_large_base.yaml"
    )
