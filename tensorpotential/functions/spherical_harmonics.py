...
class SphericalHarmonics(tf.Module):
    ...
    def build(self, float_dtype):
        ...
        self.l_tile = tf.cast(
            tf.concat([tf.ones((2 * l + 1)) * l for l in range(self.lmax + 1)], axis=0),  # noqa: E741
            self.int_dtype,
            name="l_tile",
        )
        self.is_built = True
    ...
    @staticmethod
    def lm1d(l, m):  # noqa: E741
        return m + l * (l + 1) // 2

    def lmsh(self, l, m):  # noqa: E741
        return l + abs(m) * self.lmax - abs(m) * (abs(m) - 1) // 2

    @staticmethod
    def lm_m(l, m):  # noqa: E741
        return l * (l + 1) + m
    ...
    def legendre(self, x):
        ...
        if self.lmax > 0:
            ...
            for l in range(2, self.lmax + 1):  # noqa: E741
    ...
    def _compute_sph_harm(self, rhat):
        ...
        for m in range(2, self.lmax + 1):
            ...
            for l in range(m, self.lmax + 1):  # noqa: E741
